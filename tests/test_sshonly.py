...
###########################################################################
"""Test the `SshOnlyTransport` plugin on localhost."""
import logging
import unittest

import paramiko

from aiida_sshonly.transports.sshonly import SshOnlyTransport
from aiida.transports.transport import TransportInternalError

# This will be used by test_all_plugins

plugin_transport = SshOnlyTransport(machine='localhost', timeout=30, load_system_host_keys=True, key_policy='AutoAddPolicy')
...
class TestBasicConnection(unittest.TestCase):
    ...
    def test_closed_connection_ssh(self):
        ...
        with self.assertRaises(TransportInternalError):
            transport = SshOnlyTransport(machine='localhost')
    ...
    def test_auto_add_policy():
        ...
        with SshOnlyTransport(machine='localhost', timeout=30, load_system_host_keys=True, key_policy='AutoAddPolicy'):
            pass
    ...
    def test_no_host_key(self):
        ...
        with self.assertRaises(paramiko.SSHException):
            with SshOnlyTransport(machine='localhost', timeout=30, load_system_host_keys=False):
                pass
...
def test_gotocomputer():
    """Test gotocomputer"""
    with SshOnlyTransport(machine='localhost', timeout=30, use_login_shell=False, key_policy='AutoAddPolicy') as transport:
